data:extend({
  -- aluminum ore
  {
    type = "optimized-particle",
    name = "aluminum-ore-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=1.0, g=0.5, b=0.5, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=1.0, g=0.5, b=0.5, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=1.0, g=0.5, b=0.5, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=1.0, g=0.5, b=0.5, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=1.0, g=0.5, b=0.5, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=1.0, g=0.5, b=0.5, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=1.0, g=0.5, b=0.5, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=1.0, g=0.5, b=0.5, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- chromite
  {
    type = "optimized-particle",
    name = "chromite-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.5, g=0.5, b=1.0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.5, g=0.5, b=1.0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.5, g=0.5, b=1.0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.5, g=0.5, b=1.0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.5, g=0.5, b=1.0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.5, g=0.5, b=1.0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.5, g=0.5, b=1.0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.5, g=0.5, b=1.0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- limestone
  {
    type = "optimized-particle",
    name = "limestone-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=1.0, g=1.0, b=0.8, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=1.0, g=1.0, b=0.8, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=1.0, g=1.0, b=0.8, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=1.0, g=1.0, b=0.8, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=1.0, g=1.0, b=0.8, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=1.0, g=1.0, b=0.8, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=1.0, g=1.0, b=0.8, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=1.0, g=1.0, b=0.8, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- titanium ore
  {
    type = "optimized-particle",
    name = "titanium-ore-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.6, g=0.45, b=0.45, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.6, g=0.45, b=0.45, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.6, g=0.45, b=0.45, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.6, g=0.45, b=0.45, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.6, g=0.45, b=0.45, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.6, g=0.45, b=0.45, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.6, g=0.45, b=0.45, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.6, g=0.45, b=0.45, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- magnesium-ore
  {
    type = "optimized-particle",
    name = "magnesium-ore-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.75, g=0.75, b=0.75, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.75, g=0.75, b=0.75, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.75, g=0.75, b=0.75, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.75, g=0.75, b=0.75, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.75, g=0.75, b=0.75, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.75, g=0.75, b=0.75, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.75, g=0.75, b=0.75, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.75, g=0.75, b=0.75, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- nickel-ore
  {
    type = "optimized-particle",
    name = "nickel-ore-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.3, g=0.9, b=0.61, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.3, g=0.9, b=0.61, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.3, g=0.9, b=0.61, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.3, g=0.9, b=0.61, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.3, g=0.9, b=0.61, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.3, g=0.9, b=0.61, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.3, g=0.9, b=0.61, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.3, g=0.9, b=0.61, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- lead-ore
  {
    type = "optimized-particle",
    name = "lead-ore-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.1, g=0.15, b=0.4, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.1, g=0.15, b=0.4, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.1, g=0.15, b=0.4, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.1, g=0.15, b=0.4, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.05, g=0.08, b=0.2, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.05, g=0.08, b=0.2, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.05, g=0.08, b=0.2, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.05, g=0.08, b=0.2, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- rare-earths
  {
    type = "optimized-particle",
    name = "rare-earths-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.6, g=0.3, b=1, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.6, g=0.3, b=1, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.6, g=0.3, b=1, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.6, g=0.3, b=1, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.6, g=0.3, b=1, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.6, g=0.3, b=1, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.6, g=0.3, b=1, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.6, g=0.3, b=1, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
    -- spodumene
  {
    type = "optimized-particle",
    name = "spodumene-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.5, g=1, b=0.6, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.5, g=1, b=0.6, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.5, g=1, b=0.6, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.5, g=1, b=0.6, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.5, g=1, b=0.6, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.5, g=1, b=0.6, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.5, g=1, b=0.6, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.5, g=1, b=0.6, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- spodumene
  {
    type = "optimized-particle",
    name = "spodumene-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.5, g=1, b=0.6, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.5, g=1, b=0.6, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.5, g=1, b=0.6, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.5, g=1, b=0.6, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.5, g=1, b=0.6, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.5, g=1, b=0.6, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.5, g=1, b=0.6, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.5, g=1, b=0.6, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- tinstone
  {
    type = "optimized-particle",
    name = "tinstone-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.1, g=0.9, b=0.7, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.1, g=0.9, b=0.7, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.1, g=0.9, b=0.7, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.1, g=0.9, b=0.7, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.1, g=0.9, b=0.7, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.1, g=0.9, b=0.7, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.1, g=0.9, b=0.7, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.1, g=0.9, b=0.7, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- tungsten-ore
  {
    type = "optimized-particle",
    name = "tungsten-ore-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.35, g=0.15, b=0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.35, g=0.15, b=0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.35, g=0.15, b=0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.35, g=0.15, b=0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.35, g=0.15, b=0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.35, g=0.15, b=0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.35, g=0.15, b=0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.35, g=0.15, b=0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- zinc-ore
  {
    type = "optimized-particle",
    name = "zinc-ore-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0, g=0.25, b=1, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0, g=0.25, b=1, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0, g=0.25, b=1, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0, g=0.25, b=1, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0, g=0.25, b=1, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0, g=0.25, b=1, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0, g=0.25, b=1, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0, g=0.25, b=1, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
  -- zirconium-ore
  {
    type = "optimized-particle",
    name = "zirconium-ore-particle",
    flags = {"not-on-map"},
    life_time = 180,
    pictures = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-1.png",
        priority = "extra-high",
        tint = {r=0.8, g=0.65, b=0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-1.png",
          priority = "extra-high",
          tint = {r=0.8, g=0.65, b=0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-2.png",
        priority = "extra-high",
        tint = {r=0.8, g=0.65, b=0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-2.png",
          priority = "extra-high",
          tint = {r=0.8, g=0.65, b=0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-3.png",
        priority = "extra-high",
        tint = {r=0.8, g=0.65, b=0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-3.png",
          priority = "extra-high",
          tint = {r=0.8, g=0.65, b=0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-4.png",
        priority = "extra-high",
        tint = {r=0.8, g=0.65, b=0, a=1.0},
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-4.png",
          priority = "extra-high",
          tint = {r=0.8, g=0.65, b=0, a=1.0},
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    },
  shadows = {
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-1.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-1.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-2.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-2.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-3.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-3.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      },
      {
        filename = "__Spaghenutti__/graphics/base/ore/ore-particle-shadow-4.png",
        priority = "extra-high",
        width = 16,
        height = 16,
        frame_count = 1,
        hr_version = {
          filename = "__Spaghenutti__/graphics/base/ore/hr-ore-particle-shadow-4.png",
          priority = "extra-high",
          width = 32,
          height = 32,
          frame_count = 1,
          scale = 0.5
        }
      }
    }
  },
})
